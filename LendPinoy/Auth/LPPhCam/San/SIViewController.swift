...
class SIViewController: LPBaseViewController {
    ...
    lazy var yiView: YIBanView = {
        ...
        yiView.navView.titleLabel.text = "Work Information"
        return yiView
    }()
    ...
    override func viewDidLoad() {
        ...
        siti = SystemInfo.getCurrentTime()
        HuoQiInfo()
        tapClick()
    }
}

extension SIViewController {
    ...
    func comInfo() {
        ...
        if let redict = dict {
            ...
            man.uploadDataAPI(params: redict, pageUrl: "/lpinoy/these/tasty/enjoy", method: .post) { [weak self] result in
                switch result {
                case .success(_):
                    self?.maiInfopoint("6", self?.siti ?? "", SystemInfo.getCurrentTime(), self?.chanpinID.value ?? "")
